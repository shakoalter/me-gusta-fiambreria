      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
          ...
          <motion.div
            ...
            className={`relative flex flex-col justify-between p-3 sm:p-4 rounded-2xl cursor-pointer transition-all border-2 select-none overflow-hidden ${
              isNoAderezoSelected
          ...
          >
            {/* Visual Sin Aderezo */}
            <div className="relative h-20 sm:h-24 rounded-xl overflow-hidden bg-stone-100/80 border border-stone-200/70 p-2 flex items-center justify-center mb-2.5">
              <div className="w-12 h-12 rounded-full bg-stone-200/80 flex items-center justify-center text-stone-500 shadow-inner">
                <Ban className="w-6 h-6 stroke-[2.2]" />
              </div>
              <div
                className={`absolute top-2 right-2 w-6 h-6 rounded-full flex items-center justify-center shadow-sm transition-all ${
                  isNoAderezoSelected
                    ? 'bg-amber-600 text-white'
                    : 'bg-white/90 text-stone-400 border border-stone-200'
                }`}
              >
                {isNoAderezoSelected ? <Check className="w-3.5 h-3.5 stroke-[3]" /> : <Plus className="w-3.5 h-3.5" />}
              </div>
            </div>

            {/* Info */}
            <div>
              <h5 className="font-serif font-bold text-sm sm:text-base text-gourmet-dark leading-tight">
                Sin aderezo
              </h5>
              <p className="text-[11px] sm:text-xs text-gourmet-muted mt-1 leading-snug line-clamp-2">
                Sándwich clásico sin salsas ni aderezos agregados.
              </p>
            </div>

            {/* Pie */}
            <div className="mt-3 pt-2 border-t border-gourmet-border/60 flex items-center justify-between">
              ...
              >
                {isNoAderezoSelected ? 'Seleccionado' : 'Elegir'}
              </span>
            </div>
          </motion.div>
          ...
          {ADEREZOS_DATA.map((aderezo) => {
            ...
            return (
              ...
              >
                ...
                <div className="relative h-20 sm:h-24 rounded-xl overflow-hidden bg-amber-50/40 border border-amber-200/50 p-1.5 flex items-center justify-center mb-2.5">
                  <img
                    src={aderezo.image}
                    alt={aderezo.name}
                    className="max-h-16 sm:max-h-20 w-auto object-contain transform group-hover:scale-105 transition-transform duration-300"
                    loading="lazy"
                  />
                  {aderezo.badge && (
                    <div className="absolute top-1.5 left-1.5 scale-90 sm:scale-100 origin-top-left">
          ...
          {EXTRAS_DATA.map((extra) => {
            ...
            return (
              ...
              >
                ...
                {/* Contenido */}
                <div>
                  <h5 className="font-serif font-bold text-base text-gourmet-dark leading-tight">
                    {extra.name}
                  </h5>
                  <p className="text-xs text-gourmet-muted mt-1 leading-snug line-clamp-2">
                    {extra.description}
                  </p>
                </div>
